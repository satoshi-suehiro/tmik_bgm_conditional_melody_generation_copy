...
def sequence_note_pitch_vectors(sequence, seconds_per_frame):
    ...
    #     sec_per_pos = (1/4) * orig_wav.length() / (information.bars_num * information.beats_per_bar) / orig_wav.sr
    #     frame_boundaries_sec = [sec_per_pos * pos for pos in frame_boundaries]

    #     for i, chroma in enumerate(orig_wav.chroma.T):
    #         start_sec = i * orig_wav.chroma_hop_length / orig_wav.sr
    #         start_frame = bisect.bisect_right(frame_boundaries_sec, start_sec)

    #         sorted_chroma = sorted([(pitch_class, value) for (pitch_class, value) in enumerate(chroma)],
    #                                 key = lambda x: x[1], reverse=True)

    #         for item in sorted_chroma[:ADD_N_ARGMAX_PITCH_CLASS]:
    #             pitch_class, value = item
    #             x[start_frame, pitch_class] += value
